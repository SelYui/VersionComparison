        ...
        private void FileComparison_Click(object sender, EventArgs e)
        {
            try 
            {
                ...
                else
                {
                    ...
                    if (Process.Start(@"C:\Program Files (x86)\WinMerge\WinMergeU.exe", SelectPath) == null) 
...
//*************************************************************************************************
        ...
        void AddDirectories(TreeNode node)
        {
            ...
            foreach (var d in dirInfo.GetFiles())
            {
                ...
                bool resultBAT = d.Name.EndsWith(".bat", true, ci);
                bool resultPRT = d.Name.EndsWith(".prt", true, ci);
                bool resultSET = d.Name.EndsWith(".set", true, ci);
                bool resultOBJ = d.Name.EndsWith(".obj", true, ci);
                if (resultM96 == false && resultLST == false && resultBAT == false && resultPRT == false && resultSET == false && resultOBJ == false)
